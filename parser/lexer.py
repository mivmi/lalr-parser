...
_HandlerCallback: t.TypeAlias = t.Callable[['BaseLexer', str], t.Any]
...
class Token:
    def __init__(self, name: str, value, position: t.Tuple[int, int]):
        ...
        Args:
            name (str): The name of the token (e.g., ID, INT, ...).
            value (Any): The actual value of the token.
            position (t.Tuple[int, int]): The start and end positions of the token in the line.
        """       
...
class Handler:
    """Represents a handler used to match tokens in the lexer."""

    def __repr__(self):
        return f'{self.__class__.__name__}({self.name!r})'

    def __init__(self, name: str, ignore: bool, callback: _HandlerCallback):
        self.name = name
        self.ignore = ignore
...
class MetaLexer(type):
    """
    Metaclass for creating a lexer class, automatically managing token handlers.
    """
...
class BaseLexer(metaclass=MetaLexer):
    ...
    * You can define tokens directly as constants. 
    ...
    def __init__(self, content: str, name: str = '<stdin>') -> None:
        """
        Creates a new `Lexer` instance for processing content.

        The lexer requires a text-based input (`content`) which should be a string.
        An optional `name` argument can be provided to enhance error messages.

        Args:
            content (str): The source to tokenize.
            name (str, optional): The name of the input source. Defaults to '<stdin>'.
        """
    ...
    def read(self, length: int):
        ...
        This method returns the substring of the specified length from the current position in the content.

        Args:
            length (int): The number of characters to read from the content.

        Returns:
            str: The substring read from the content.
        """
    ...
    def iter(self):
        """
        Generates the next character from the `content`, one at a time.

        This method is used internally to iterate through input characters lazily.

        Yields:
            str: The next character from the content.
    ...
    def readline(self):
        """
        Reads a line of text from the content.

        This method finds the next newline character and returns the substring from the current position
        to the position of the newline. If no newline is found, it returns the rest of the content.

        Returns:
            str: The line of text read from the content.
        """
    ...
    def tokenize(self):
        """
        Tokenizes the entire content and returns all tokens in order.

        This method repeatedly calls `get_next_token` until the end of the input is reached (`$EOF`).
        The resulting tokens are stored in a `deque` and returned to the caller.

        Returns:
            deque['Token']: An ordered collection of tokens identified in the content.
        """
        result = deque()
        while True:
    ...
    def set_position(self, position: int, current: bool = True):
        """
        Adjusts the reading position within the content.

        The `position` can be absolute or relative based on the `current` argument.
        Args:
            position (int): The position or relative position.
            current (bool, optional): If `True`, `position` is relative to the current position. Defaults to `True`.
    ...
    def get_next_token(self) -> 'Token':
        """
        Retrieves the next valid token from the input content.

        This method iterates over the input to match the longest valid token using
        registered handlers and a state-based transition graph. If no valid token is
        matched, a `SyntaxError` is raised.

        Raises:
            SyntaxError: If no valid token is matched at the current input position.

        Returns:
            Token: The next valid token found in the content, or an EOF token if the
        ...
        # Check for a valid token match using the DAWG (Directed Acyclic Word Graph).
        state_id = 0
        rollback = False
        response = ''
...
def token(name: str, *, ignore: bool=False):
    ...
    """
    def wrapper(callback: '_HandlerCallback'):
